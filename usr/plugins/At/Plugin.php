...
class At_Plugin implements Typecho_Plugin_Interface
{
    ...
    public static function footer($widget) 
    {
        ...
        foreach($comments as $comment)
        {
        	...
        	$data[] = array('id'=>$comment['coid'], 'name'=>$comment['author'], 'text'=>$comment['author'].$comment['text']);
        }
        echo '<link rel="stylesheet" type="text/css" href="' .Helper::options()->pluginUrl.'/At/res/css/jquery.atwho.css">';
        echo '<script type="text/javascript" src="'.Helper::options()->pluginUrl.'/At/res/js/jquery.atwho.min.js"></script>';
    ?>
            <script>
		//<![CDATA[
		TypechoComment.reply = function (cid, coid) {
                ...
                // 如果是独立页面, 以上面的代码， 取出来的response是null
                if(response == null) {
                    response = TypechoComment.dom('respond-page-<?php echo $widget->cid; ?>');
                }
                // console.log(response);
