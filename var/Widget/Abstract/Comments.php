    ...
    public function gravatar($size = 32, $default = NULL)
    {
        if ($this->options->commentsAvatar && 'comment' == $this->type) {
            ...
            if (!$plugged) {
                ...
                // 根据是否为qq邮箱判断使用qq头像还是gravatar头像
                if(preg_match_all('|^([1-9]\d{4,10})@qq\.com$|i', $this->mail, $res)) {
                    echo '<img class="avatar" src="http://qqimg.wg1997.cn/qqimg.php?qq='.$res[1][0].'" alt="' .
                    $this->author . '" width="' . $size . '" height="' . $size .'" />';
                }
                else if(preg_match("/^[_a-z0-9-]+(\.[_a-z0-9-]+)*@[a-z0-9-]+(\.[a-z0-9-]+)*(\.[a-z]{2,})$/", $this->mail)){
